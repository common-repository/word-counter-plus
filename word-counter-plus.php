...
// Load text domain
function wcpmo_wordcount_load_textdomain()
{
    load_plugin_textdomain('word-counter-plus', false, dirname(__FILE__) . '/languages');
}
add_action("plugins_loaded", "wcpmo_wordcount_load_textdomain");

// Add a new column in the admin post list
function wcpmo_custom_posts_columns($columns)
{
    $columns['word_count'] = __('Word Count', 'word-counter-plus');
    return $columns;
}
add_filter('manage_posts_columns', 'wcpmo_custom_posts_columns');

// Fill the new column with the word count for each post
function wcpmo_custom_posts_custom_column($column_name, $post_id)
{
    if ('word_count' === $column_name) {
        $content = get_post_field('post_content', $post_id);
        $word_count = wcpmo_custom_count_words($content);
        echo esc_attr($word_count);
    }
}
add_action('manage_posts_custom_column', 'wcpmo_custom_posts_custom_column', 10, 2);

// Function that closely mimics the WordPress editor word count
function wcpmo_custom_count_words($content)
{
...
function wcpmo_custom_posts_column_sortable($columns)
{
    $columns['word_count'] = 'word_count';
    return $columns;
}
add_filter('manage_edit-post_sortable_columns', 'wcpmo_custom_posts_column_sortable');
